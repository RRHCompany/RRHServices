 ...
 * 手机接口  参数
 * */
public class MobileParameterFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res,FilterChain chain) throws IOException, ServletException {
		...
		if(StringUtils.isEmpty(appSystem) || StringUtils.isEmpty(appDeviceInfo) || StringUtils.isEmpty(appChannel)){
			BaseResponse baseResponse = new BaseResponse(ConstantsMobile.RESULT_CODE_ERROR_PUBLIC_PARAM);
		...
		if(!SystemConstant.LOGINFILTER_METHOD_FREE.contains(path)){
			// 校验token是否合法
			JSONObject errorJson = (JSONObject) JSON.toJSON(ResultMobile.resultFall("user.token.verify"));
