...
import com.rrh.common.utils.MD5Util;
...
public class UserService extends BaseService<TbUsers> implements IUserService<TbUsers> {
	 ...
	 * @param password
	 * @return
	 * http://10.0.8.236:8080/rrh/mobile/v1.0/users/login?appChannel=android&appSystem=android&password=123456&appDeviceInfo=867064018359594&mobile=13538023252
	 */
	public BaseResponse phoneLogin(String mobile, String password){
		...
		if(!RegexpUtil.isMobile(mobile)){
			return ResultMobile.resultFall("user.mobile.error");
		}
		try {
	...
	public BaseResponse verifyUserByUserName(String userName){
		...
		try {			
			...
			//用户名已注册
			if(userId > 0){
				return new BaseResponse(ErrorMsgConstant.USER_NAME_REPEAT_ERRORCODE);
			}else{
		...
		try {
			...
			if(verifyCode_return != 1){
				if(verifyCode_return == -4){
					...
					return ResultMobile.resultFall("sms.code.error");
				}
			}
			String mobile = CommonUtils.encordSQL(bean.getMobile(),0);
			//查询用户名是否存在
			int userId = tbUsersMapper.selectUserIdByMobile(mobile);
			//用户名已注册
			if(userId > 0){
	...
	public BaseResponse getUserInfo(String userId){
		...
		try{
			...
			if(tbUsers != null){
				...
				return new BaseResponse(ConstantsMobile.RESULT_CODE_FAIL);			
		}catch(Exception e){
			log.error("", e);
			return new BaseResponse(ConstantsMobile.RESULT_CODE_ERROR_SYSTEM);
		}
