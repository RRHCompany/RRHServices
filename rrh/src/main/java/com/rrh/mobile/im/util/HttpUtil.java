package com.rrh.mobile.im.util;
...
public class HttpUtil {
	...
	// 设置body体
	public static void setBodyParameter(Map<String, String> paramsMap, HttpURLConnection conn)throws IOException {
		DataOutputStream out = new DataOutputStream(conn.getOutputStream());
		out.writeBytes(HttpUtil.getParameter(paramsMap));
		out.flush();
		out.close();
	}
	//将参数转换成 链接字符串
	private static String getParameter(Map<String, String> paramsMap) throws UnsupportedEncodingException {
		...
		for (String key : keys) {
			...
			params.append(key).append("=").append(URLEncoder.encode(paramsMap.get(key), IM.UTF8)).append("&");
		}
	...
	public static HttpURLConnection CreatePostHttpConnection(String uri) throws MalformedURLException,IOException, ProtocolException {
		...
		StringBuilder toSign = new StringBuilder(IM.APP_SECRE).append(nonce).append(timestamp);
		...
		conn.setRequestProperty(APPKEY, IM.APP_KEY);
