		...
		try{
			...
			if(flag){
				...
				if(result != null && result != -1){					
					...
					return new BaseResponse(ErrorMsgConstant.CREATE_VERIFY_MOREREQUEST_ERRORCODE, SystemConstant.DEFAULT_REQUEST_TIME);
				}
			}else{
				return new BaseResponse(type.equals("1")?ErrorMsgConstant.PHONE_HAD_RESGISTER_ERRORCODE:ErrorMsgConstant.EXTEND_ERRORCODE, SystemConstant.DEFAULT_REQUEST_TIME);
			}
		}catch(Exception e){
			log.error("", e);
